...
export default function BidConfirmModal({ open, item, bidAmount, loading, onClose, onConfirm }: BidConfirmModalProps) {
  if (!open || !item) return null;
  const fee = Math.round(bidAmount * 0.02);
  const total = bidAmount + fee;
  return (
    <div className="bm-modal-backdrop" onClick={loading ? undefined : onClose}>
      <div className="bm-modal" onClick={e => e.stopPropagation()}>
        <div className="bm-modal-head">
          <h3>Konfirmasi tawaran kamu</h3>
          <button className="bm-modal-close" onClick={onClose} disabled={loading}><X width={20} height={20}/></button>
        </div>
        ...
        <div className="bm-modal-foot">
          <Button variant="secondary" size="lg" onClick={onClose} disabled={loading}>Batal</Button>
          <Button variant="primary" size="lg" onClick={onConfirm} loading={loading}>
