...
export function mapListingToAuctionItem(listing: Listing): AuctionItem {
  return {
    id: listing.id,
    title: listing.title,
    price: listing.currentPrice,
    bids: 0, // Should be fetched from Auction service if needed
    ends: new Date(listing.endTime).getTime(),
    art: getArtForCategory(listing.category.name),
    cat: listing.category.id,
    seller: listing.sellerId,
    rating: 4.8, // Mock
    ratingCount: 120, // Mock
  };
}

function getArtForCategory(name: string): string {
  const map: Record<string, string> = {
    'Elektronik': 'bm-art-elec',
    'Fashion': 'bm-art-fash',
    'Otomotif': 'bm-art-veh',
    'Koleksi & Antik': 'bm-art-coll',
    'Rumah & Taman': 'bm-art-home',
  };
  return map[name] || 'bm-art-toys';
}
