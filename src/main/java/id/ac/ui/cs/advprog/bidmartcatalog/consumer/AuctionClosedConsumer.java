...
/**
 * Consumer async untuk event AuctionClosed (status = UNSOLD).
 *
 * Tanggung jawab:
 ...
 *
 * Jaminan spec: setiap penutupan lelang hanya menghasilkan satu event
 * (AuctionClosed ATAU WinnerDetermined), sehingga idempotency cukup dijaga
 * via eventId.
 */
@Slf4j
@Component
...
public class AuctionClosedConsumer {
    ...
    @RabbitListener(
        queues = RabbitMQConfig.QUEUE_AUCTION_CLOSED,
